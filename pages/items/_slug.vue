    ...
    <div class="pt-3">
        ...
        <div class="row">
            <div class="col-xl-3 col-md-4 mb-1">
                <div class="card shadow">
                    ...
                    <div class="card-body p-4"
                        :style="`background-image: url('${item.icon}');background-repeat: no-repeat;background-position: center center;background-size: 8em auto;image-rendering: pixelated;min-height: 10em;`">
                        ...
                        <table class="table table-striped borderless">
                            ...
                            <tbody>
                                <template v-if="item.category == 'weapon'">
                                    <tr>
                                        <td>Atk</td>
                                        <td>{{ item.minAttack }} - {{ item.maxAttack }}</td>
                                    </tr>
                                </template>
                                <template v-if="item.category == 'armor'">
                                    <tr>
                                        <td>Def</td>
                                        <td>{{ item.minDefense }} - {{ item.maxDefense }}</td>
                                    </tr>
                                </template>
                                <tr>
                                    <td>Buy Price: </td>
                                    <td>{{ item.sellPrice | penya }}</td>
                                </tr>
                                <tr>
                                    <td>Sell Price: </td>
                                    <td>{{ item.sellPrice / 4 | penya }}</td>
                                </tr>
                                            ...
                                            <ul class="mb-0 pl-4">
                                                <li v-for="itemability in item.abilities" :key="itemability.parameter">
                                                    ...
                                                    <template v-if="itemability.add">
                                                        + {{ itemability.add }}<template
                                                            v-if="itemability.rate">%</template>
                                                    </template>
            ...
            <div class="col-xl-9 col-md-8 mb-8">
                        ...
                        <nuxt-link :to="`/quests/${currentQuest.flyffdb_meta_id}`">
                            <div class="callout callout-success">
                                <h5 class="text-info">
                                    ...
                                    <sub class="text-warning">Level {{ currentQuest.minLevel }} - {{ currentQuest.maxLevel
                                    }}</sub>
                                </h5>
                                        ...
                                        <table class="table table-striped">
                                            <tbody>
                                                <tr v-for="currentMonster in droppingMonsters" :key="currentMonster.id">
                                                    <td>
                                                        <nuxt-link :to="`/monsters/${currentMonster.flyffdb_meta_id}`"
                                                            class="user-block">
                                                            <img :src="`${currentMonster.icon}?tr=w-100,h-100,cm-pad_resize`"
                                                                alt="Monster Icon">
                                                            <span class="username text-muted">{{ currentMonster.name.en
                                                            }}</span>
                                                            <span class="description">Level: {{ currentMonster.level
                                                            }}</span>
                                                        </nuxt-link>
                                                    </td>
                                                </tr>
                ...
                <h2>Player shops:</h2>
                <div class="row">
                    <p v-if="$fetchState.pending">Fetching player shop data...</p>
                    <p v-else-if="$fetchState.error">Error while fetching player shop data</p>
                    <template v-if="droppingMonsters.length">
                        <div class="col-xl-6 col-md-6">
                            <div class="card">
                                <div class="card-header">
...
export default {
    async asyncData({ $http, params, error }) {
        const { item, quests } = await $http.$get(`/api/items/${params.slug}`);
